...
class FeedbackController extends BaseController {
	...
	public function mgr($o_keyword='', $o_status='', $sort='', $startdate='', $enddate=''){
		...
		if(!empty($o_keyword)){
			$map['message'] = array('like', '%'.$o_keyword.'%');
			$map['company'] = array('like', '%'.$o_keyword.'%');
			$map['phone'] = array('like', '%'.$o_keyword.'%');
			$map['email'] = array('like', '%'.$o_keyword.'%');
			$map['subject'] = array('like', '%'.$o_keyword.'%');
		}

		/** @var  $feedbackM  \Think\Model */
		$feedbackM = M("Feedback");
		$count      = $feedbackM->where($map)->count();// 查询满足要求的总记录数
		...
		$list = $feedbackM->where($map)->order('`createtime` '.$sort)->limit($page->firstRow.','.$page->listRows)->select();
		$commentList = [];
